...
//   return (
//     <div className="cart">
//       <h2>Danh sách các món trong giỏ hàng:</h2>
//       <div>
//         {/* <h3></h3> */}
//         {Object.keys(orders).length > 0 ? (
//           Object.keys(orders).map((restaurantId) => (
...
const Cart = ({ customerId }) => {
  ...
  const [showDetails, setShowDetails] = useState(false); // State kiểm soát hiển thị chi tiết đơn hàng
  const token = localStorage.getItem("access_token");
  ...
  const fetchOrderDetails = async (orderId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setShowDetails(true); // Khi bấm vào "Xem chi tiết" thì hiển thị chi tiết
      } else {
        console.error("API trả về lỗi:", response.status);
      }
    } catch (error) {
  ...
  return (
    <div className="cart">
      <h2>Giỏ Hàng</h2>
      <div className={`cart-container ${showDetails ? "show-details" : ""}`}>
        {/* Danh sách các món */}
          ...
          <ul>
            ...
            {selectedOrderDetails?.length > 0 && (
              <li>
                <p>
                  Phí đơn hàng:{" "}
                  {orderFees[selectedOrderDetails[0].order_id] || "Đang tải..."} VND
                </p>
              </li>
            )}
            <button onClick={() => handlePlaceOrder(selectedOrderDetails[0].order_id)}>
              Đặt hàng
            </button>
          </ul>
        </div>
