...
const SignUpPopup = ({ setShowSignUp }) => {
  ...
  const [errors, setErrors] = useState({});
  const [loading, setLoading] = useState(false); // Để theo dõi trạng thái đang tải
  const [success, setSuccess] = useState(false); // Theo dõi việc đăng ký thành công

  useEffect(() => {
    if (success) {
      setShowSignUp(false); // Đóng popup khi đăng ký thành công
      resetForm(); // Reset form sau khi thành công
    }
  }, [success, setShowSignUp]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0 && !loading) {
      ...
      try {
        ...
        console.error(error);
        handleError(error); // Xử lý lỗi trả về từ server
      } finally {
        setLoading(false); // Dừng trạng thái loading
      }
    }
